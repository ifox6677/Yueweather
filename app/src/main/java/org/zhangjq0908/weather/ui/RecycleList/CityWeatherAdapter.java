...
public class CityWeatherAdapter extends RecyclerView.Adapter<CityWeatherAdapter.ViewHolder> {
    ...
    private List<HourlyForecast> courseDayList = new ArrayList<>();
    private List<WeekForecast> weekForecastList = new ArrayList<>();
    private List<QuarterHourlyForecast> quarterHourlyList = new ArrayList<>();
    ...
    public CityWeatherAdapter(CurrentWeatherData currentWeatherDataList, int[] dataSetTypes, Context context) {
        ...
        quarterHourlyList = database.getQuarterHourlyForecastsByCityId(currentWeatherDataList.getCity_id());
    ...
    public void onBindViewHolder(ViewHolder viewHolder, final int position) {
        ...
        if (viewHolder.getItemViewType() == OVERVIEW) {
            ...
            if (quarterHourlyList.isEmpty()){
                ...
                if (nowCast!=null) {
                    if ((riseTime - setTime) % 86400000 == 0) holder.sun.setText("\u2600\u25b2 --:--" + " \u25bc --:--" );
                    else  {
                ...
                for (QuarterHourlyForecast f : quarterHourlyList) {
            ...
            holder.recyclerView.addOnItemTouchListener(
                    new RecyclerItemClickListener(context, holder.recyclerView, new RecyclerItemClickListener.OnItemClickListener() {
                        ...
                        public void onItemClick(View view, int position) {
                            ...
                            if (weekForecastList == null || position >= weekForecastList.size()) return;
                            long time = weekForecastList.get(position).getForecastTime();  //time of clicked week item
